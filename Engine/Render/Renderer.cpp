...
namespace Wanted
{
	...
	void Renderer::Draw(float deltaTime)
	{
		...
		for (const RenderCommand& command : renderQueue)
		{
			...
			if (command.isUI)
			{
				// UI인 경우 카메라 변환을 거치지 않고 절대 좌표 사용
				screenPos = command.position;
			}
			else
			{
				// 월드 좌표 - 카메라 좌표 + 화면 중심 오프셋 = 화면 상대좌표 계산
				screenPos = command.position - effecttiveCamPos + screenCenter;
			}
